###
# Largerly based on https://github.com/diffusion-hyperfeatures/diffusion_hyperfeatures
...
from archs.stable_diffusion.diffusion import (
    init_models_controlnet, 
    get_tokens_embedding,
    generalized_steps,
    collect_and_resize_feats
)
from archs.stable_diffusion.resnet import init_resnet_func
from archs.stable_diffusion.diffusion import freeze_weights

class DiffusionExtractorControlNet(): # torch.nn.Module):
    """
    Module for running either the generation or inversion process and extracting intermediate feature maps.
    """
    def __init__(self, config):
        ...
        self.unet, self.vae, self.clip, self.clip_tokenizer, self.controlnet, self.scheduler, self.pipe = init_models_controlnet(config["model_list"]) 
        #model_id=config["model_id"])
        
        self.num_timesteps = config["num_timesteps"]
        self.scheduler.set_timesteps(self.num_timesteps)
        self.generator = torch.Generator().manual_seed(config.get("seed", 0))
    ...
    def init_extractor(self, device):
        self.unet   = self.unet.to(device)
        self.vae    = self.vae.to(device) 
        self.clip   = self.clip.to(device)
        self.controlnet = self.controlnet.to(device)
        self.pipe = self.pipe.to(device)
        self.change_cond(self.prompt, "cond", device)
        self.change_cond(self.negative_prompt, "uncond", device)

    def change_controlnets(self, module_list, device):
        controlnets = []
        for model_link in module_list:
            controlnets.append(
    ...
    def change_cond(self, prompt, cond_type="cond", device=None):
        with torch.no_grad():
            with torch.autocast("cuda"):
                _, new_cond = get_tokens_embedding(self.clip_tokenizer, self.clip, prompt, device)
    ...
    def change_cond_emb(self, prompt_emb, prompt, cond_type="cond"):
        with torch.no_grad():
            with torch.autocast("cuda"):
                if cond_type == "cond":
                    self.cond = prompt_emb
                    self.prompt = prompt
                elif cond_type == "uncond":
                    self.uncond = prompt_emb
                    self.negative_prompt = prompt
                else:
                    raise NotImplementedError
    ...
    def get_feats(self, latents, extractor_fn, save_timestep, preview_mode=False):
        # returns feats of shape [batch_size, num_timesteps, channels, w, h]
        if not preview_mode:
            init_resnet_func(self.unet, save_hidden=True, reset=True, idxs=self.idxs, save_timestep=save_timestep)
        outputs = extractor_fn(latents)
        if not preview_mode:
        # if False:
            feats = []
            for timestep in save_timestep:
                timestep_feats = collect_and_resize_feats(self.unet, self.idxs, timestep, resolution=64)
    ...
    def forward(self, 
        ...
        if images is None:
            ...
            if self.diffusion_mode == "generation":
                if inpaint_img is not None and inpaint_mask is not None:
                    extractor_fn = lambda latents: self.run_generation(
                        latents, 
                        guidance_scale, 
                        control_image, 
                        cond_scale=cond_scale, 
        ...
        with torch.no_grad():
            with torch.autocast("cuda"):
                if sdedit_img_latent is None:
                    save_ts = self.save_timestep
                else:
                    init_timestep = min(
                        int(self.scheduler.num_inference_steps * pipeline_quant), 
                        self.scheduler.num_inference_steps)
                    t_start = max(self.scheduler.num_inference_steps - init_timestep, 0)
                    min_ts = np.array([ts for ts in self.save_timestep if ts>=t_start]).min()
                    save_ts = [
                        ts for ts in self.save_timestep if ts>=t_start] + [
                            min_ts for ts in self.save_timestep if ts<t_start]
                return self.get_feats(latents, extractor_fn, save_timestep=save_ts, preview_mode=preview_mode)